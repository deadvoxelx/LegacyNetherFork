...
bool NetherFurnaceTile::noDrop = false;

NetherFurnaceTile::NetherFurnaceTile(int id, bool lit) : BaseEntityTile(id, Material::stone)
{
	random = new Random();
	this->lit = lit;
...
int NetherFurnaceTile::getResource(int data, Random *random, int playerBonusLevel)
{
	return Tile::nether_furnace_Id;
}

void NetherFurnaceTile::onPlace(Level *level, int x, int y, int z)
{
	BaseEntityTile::onPlace(level, x, y, z);
	recalcLockDir(level, x, y, z);
}

void NetherFurnaceTile::recalcLockDir(Level *level, int x, int y, int z)
{
...
Icon *NetherFurnaceTile::getTexture(int face, int data)
{
...
void NetherFurnaceTile::registerIcons(IconRegister *iconRegister)
{
	icon = iconRegister->registerIcon(L"nether_furnace_side");
	iconFront = iconRegister->registerIcon(lit ? L"nether_furnace_front_lit" : L"nether_furnace_front");
	iconTop = iconRegister->registerIcon(L"nether_furnace_top");
}

void NetherFurnaceTile::animateTick(Level *level, int xt, int yt, int zt, Random *random)
{
	if (!lit) return;
...
bool NetherFurnaceTile::TestUse()
{
	return true;
}

bool NetherFurnaceTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) // 4J added soundOnly param
{
...
void NetherFurnaceTile::setLit(bool lit, Level *level, int x, int y, int z)
{
	int data = level->getData(x, y, z);
	shared_ptr<TileEntity> te = level->getTileEntity(x, y, z);

	noDrop = true;
	if (lit) level->setTileAndUpdate(x, y, z, Tile::nether_furnace_lit_Id);
	else level->setTileAndUpdate(x, y, z, Tile::nether_furnace_Id);
	noDrop = false;
...
shared_ptr<TileEntity> NetherFurnaceTile::newTileEntity(Level *level)
{
	return std::make_shared<FurnaceTileEntity>();
}

void NetherFurnaceTile::setPlacedBy(Level *level, int x, int y, int z, shared_ptr<LivingEntity> by, shared_ptr<ItemInstance> itemInstance)
{
...
void NetherFurnaceTile::onRemove(Level *level, int x, int y, int z, int id, int data)
{
	if (!noDrop)
	{
...
bool NetherFurnaceTile::hasAnalogOutputSignal()
{
	return true;
}

int NetherFurnaceTile::getAnalogOutputSignal(Level *level, int x, int y, int z, int dir)
{
	return AbstractContainerMenu::getRedstoneSignalFromContainer(dynamic_pointer_cast<Container>( level->getTileEntity(x, y, z)) );
}

int NetherFurnaceTile::cloneTileId(Level *level, int x, int y, int z)
{
	return Tile::nether_furnace_Id;
}